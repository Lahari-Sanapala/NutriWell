...
export default function NutriWellHomeScreen() {
  ...
  const fetchDailyTotals = async (currentUserId) => {
    ...
    try {
      const response = await fetch(`http://192.168.146.176:5000/api/details/${currentUserId}/daily-totals`);
  ...
  const fetchWeeklyTotals = async (currentUserId) => {
    if (!currentUserId) return;
    try {
      const response = await fetch(`http://192.168.146.176:5000/api/details/${currentUserId}/weekly-totals`);
      const data = await response.json();
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        const response = await fetch(`http://192.168.146.176:5000/api/auth/${userId}/basic-info`);
        const data = await response.json();
