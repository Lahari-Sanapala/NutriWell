...
export default function editDetails() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.137.154:5000/api/details/submit", {
        method: 'POST',
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log('storedid from editDetails', storedId)
      if (storedId) {
        console.log("User ID from editDetails:", storedId);
        setUserId(storedId);
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        const response = await fetch(`http://192.168.137.154:5000/api/details/${userId}/edit-details`);
